...
import EchartsWidget, { BarChartWidget } from "insight-widgets-echarts";
...
import { emcaide as EmcaIdeView } from "./AppsView";
...
const widgetMappings = {
    "BarChartWidget": EchartsWidget({ type: "bar" }),
	"InvertedBarChart": InvertedBarChart(),
	"InvertedBarChartError": InvertedBarChart( { type: "error" }),
    "LineChart": EchartsWidget({ type: "line"}),
};

export class AppsEmcaIdeApp extends Component {
    constructor(props) {
        super(props);
		
		this.props.dispatch(actions.replace(EmcaIdeView));
...
export default connect(mapStoreToProps)(AppsEmcaIdeApp);
